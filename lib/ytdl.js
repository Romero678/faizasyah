...
const mbuhlah = /(?:http(?:s|):\/\/|)(?:(?:www\.|)youtube(?:\-nocookie|)\.com\/shorts\?feature=|\/)([^\s&]+)/
//fucntion
...
const ytshort = async (url) => {
   return new Promise((resolve, reject) => {
        if (mbuhlah.test(url)) {
            let ytId = mbuhlah.exec(url)
            ...
            post('https://www.y2mate.com/mates/en60/analyze/ajax', {
                ...
                .then(res => {
                    ...
                    post('https://www.y2mate.com/mates/en60/convert', {
                        ...
                        .then(res => res.json())
                        .then(res => {
                            KB = parseFloat(filesize) * (1000 * /MB$/.test(filesize))
                            resolve({
                                dl_link: /<a.+?href="(.+?)"/.exec(res.result)[1],
                                thumb,
                                title,
