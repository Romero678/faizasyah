...
function tebakgambar() {
    ...
    result.img = link2 + $(b).find('img').attr('data-src')
    result.jwb = $(b).find('img').attr('alt')
    /*result.push({
    	message: 'By IXB TEAM',
    	image: img,
    	jawaban: jwb
...
function ighighlight(username){
	return new Promise(async(resolve, reject) => {
		axios.request({
			url: 'https://www.instagramsave.com/instagram-highlights-downloader.php',
			method: 'GET',
			headers:{
				"user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36",
				"cookie": "PHPSESSID=ugpgvu6fgc4592jh7ht9d18v49; _ga=GA1.2.1126798330.1625045680; _gid=GA1.2.1475525047.1625045680; __gads=ID=92b58ed9ed58d147-221917af11ca0021:T=1625045679:RT=1625045679:S=ALNI_MYnQToDW3kOUClBGEzULNjeyAqOtg"
...
function fbdown(link){
	...
	axios.post('https://www.getfvid.com/downloader',qs.stringify(config),{
			headers: {
